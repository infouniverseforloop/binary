/* public/script.js — UI + WebSocket + TTS + sparkline + small dashboard */
...
function showHold(data){
  ...
  stat_conf.innerText = `Confidence: —`;
  stat_mode.innerText = `Mode: —`;
  stat_mtg.innerText = `MTG: —`;
}
...
function showSignal(rec){
  ...
  signalBody.innerHTML = `<div>Entry price: <span class="confidence">${rec.entry}</span>${candleTag}</div>${analysis}${mtgLine}`;

  stat_conf.innerText = `Confidence: ${rec.confidence}%`;
  stat_mode.innerText = `Mode: ${rec.mode || '—'}`;
  ...
  signalTimes.innerHTML = `Server(UTC): ${serverEntryIso || '—'} • Local: ${localEntry} • BD: ${toBangladesh(serverEntryIso)}`;
  ...
  pushLog(`Signal: ${rec.symbol} ${rec.direction} conf:${rec.confidence}% entry:${rec.entry} candle:${rec.candleSize || '-'}`);
}
